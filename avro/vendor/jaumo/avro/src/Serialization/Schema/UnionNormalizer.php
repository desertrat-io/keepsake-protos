...
final class UnionNormalizer implements Normalizer, NormalizerAware
{
    use HasNormalizer;

    public function supportsNormalization(Schema $schema): bool
    {
        return $schema instanceof Union;
    }

    public function normalize(Schema $schema, bool $canonical = false, ?Context $context = null)
    {
        if (!$schema instanceof Union) {
            throw NormalizationError::unsupportedType(\gettype($schema), Union::class);
        }

        return \array_map(
            function (Schema $schema) use ($canonical, $context) {
                return $this->normalizer->normalize($schema, $canonical, $context);
            },
            $schema->getTypes()
        );
    }
